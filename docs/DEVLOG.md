...
## Update 2026-03-06 (Палитра, UI, карточки)

### Палитра
- **Фон:** `#4D4D4D` (было `#004166` → `#333333`).
- **Интерактивные кнопки и ссылки:** `#AFFC41`.
- **Теги:** `#FF99E5` (PRODUCT DESIGNER, теги проектов).
- **Текст в кнопках и тегах:** `#00060A`.
- **Ссылки TELEGRAM/EMAIL:** полностью залиты `#AFFC41`.
- **Свечение за головой:** белое (было синее/зелёное).
- **3D-сцена:** освещение `#AFFC41`.

### Фон (`SiteBackground.tsx`)
- Сетка: шаг 60px (было 95.24px).
- Пятно только на главной (`/` и `/en`), на остальных страницах — только сетка.
- Убраны тёмные тени у стеклянных компонентов.

### Кнопка «Проекты»
- Залитая (не стеклянная): `bg-[#AFFC41]`, текст `#00060A`, иконка — круг `#00060A` со стрелкой белой.

### Смена языка
- Активный язык: залит `#AFFC41`, текст `#00060A`.
- Неактивный: прозрачный, hover → `#AFFC41`.

### Теги и кнопки
- Толщина шрифта: `font-light` в тегах, кнопках, ссылках.
- У тегов убрана обводка (`border`).

### Карточки проектов
- Hover: полный зелёный фон `#AFFC41`, текст `#00060A`.
- Теги при hover: `bg-[#00060A]`, текст `#AFFC41`.
- Область изображения при hover: градиент → зелёный.

---

## Update 2026-03-06 (сессия 2: карточки, мобилка, деталка)

### Палитра
- **Фон:** `#4D4D4D` → `#333333` везде (SiteBackground, globals.css, карточки, ProjectDetailView).

### Карточки проектов
- Без обводки у карточек (border убран).
- Фон карточек: `bg-[#333333]` (как фон сайта).
- Обводка: `border-white/30`, при hover `border-[#AFFC41]` (сливается с фоном).
- Текст: белый (`text-white`, `text-white/70`).
- Теги: без изменений (`bg-[#FF99E5]`, `text-[#00060A]`).
- Сетка на фоне: возвращена на всех страницах (в т.ч. `/projects`).

### Типографика
- Минимальная толщина текста: `font-light` по всему проекту (app, components).
- Hero-заголовок «ВАНЯ РОССОЛОВ»: `font-semibold` (исключение).
- body: `font-light` по умолчанию в globals.css.

### Кнопки TELEGRAM / EMAIL
- Обводка `border-[#affc41]` сохранена (на главной и в деталке).
- Мобилка: визуал как на десктопе (обводка, иконка стрелки).

### Кнопка «Проекты» (главная)
- Горизонтальные отступы: +12px (`px-10` desktop, `px-9`/`px-8` mobile).
- Мобилка: шрифт как у TELEGRAM — `clamp(14px, 4vw, 16px)`.
- Мобилка: вертикальные отступы +4px (`py-5`/`py-4`).

### Деталка проекта — правый сайдбар (desktop)
- Карточка «к проектам»: сплошной фон `bg-[#333333]`, обводка `border-white/30`.
- Разделитель убран (desktop).

### Деталка проекта — мобилка
- Два отдельных бара вместо одного: бар с контентом сцены (название + описание) и бар с контроллами (к проектам, prev/next, название проекта, «в составе»).
- Отступ между барами: 16px (`gap-4`).
- Вертикальные отступы внутри баров: 24px (`py-6`).
- Бар с контентом: без обводки, без фона (прозрачный).
- Бар с контроллами: обводка `border-white/30`, фон `bg-[#333333]`.
- Разделитель под названием проекта (только мобилка), отступы `mt-2` / `mt-4`.

### Раздел проектов — хедер
- Бар «На главную»: отступы сверху и снизу по 16px (`pt-[calc(1rem+env(safe-area-inset-top))]`, `pb-4`).

---

## Update 2026-03-06 (сессия 3: UI-правки кнопок, текст, мобилка)

### Кнопка «Проекты» (главная)
- Кружок со стрелкой: непрозрачность 70% (`bg-[#00060A]/70`).

### Навигационные ссылки
- «На главную» (ProjectsPageHeader): зелёный цвет `text-[#AFFC41]`.
- «К проектам» (деталка): зелёный цвет `text-[#AFFC41]` (desktop и mobile).

### Текст
- Все оттенки белого (`text-white/70`, `text-white/80`, `text-white/90`, `text-white/95`) заменены на `text-white` (app, components, Scene3D).

### Деталка проекта — мобильные переключалки
- Стиль: сплошной белый с непрозрачностью 30% (`bg-white/30`), стрелки белые, без обводки.
- Размер кнопок: 32×32px (`h-8 w-8`).
- Hover/active: `hover:bg-white/50 active:bg-white/50` + `transition-colors`.
- Ссылка «К проектам»: fill по высоте (`h-full self-stretch`), родитель `items-stretch`.

---

## Update 2026-03-06 (сессия 4: layout, бары, мобилка, active)

### Главная — мобилка (`app/page.tsx`, `app/en/page.tsx`)
- **Без скролла:** `overflow-hidden`, сцена 3D растягивается/сжимается (`flex-1 min-h-0`).
- Кнопка «Проекты» в потоке контента (не fixed), отступ сверху 56px (`mt-14`).
- Нижний паддинг контейнера: 20px + safe-area.
- Заголовок «ВАНЯ РОССОЛОВ» / «IVAN ROSSOLOV» по центру (`text-center`).

### Главная — десктоп
- Кнопка «Проекты» и переключатель языка вынесены из absolute/fixed в отдельный контейнер под контентом.
- Flex: контент (`flex-1`) + контроллы (`shrink-0`) с `justify-between`.

### Раздел проектов (`/projects`, `/en/projects`)
- Контейнер карточек: `max-w-[1400px]`, `px-4 md:px-8` (как на главной).
- Бар «Назад» перенесён вниз (fixed bottom), стиль как в деталке: `rounded-[20px]` → `rounded-t-[20px]`, `border-t border-x`, `bg-[#333333]`.
- **Мобилка:** бар упирается в края (без px у родителя), без скруглений снизу.
- **Десктоп:** без скруглений снизу у бара.

### Деталка проекта (`components/ProjectDetailView.tsx`)
- **Мобилка:** контейнер контента слайда — без скруглений и паддингов.
- Отступ между контентом слайда и нижним баром: 24px (`gap-6`).
- Нижний паддинг бара: 32px + safe-area.
- **Десктоп:** бар прижат к низу (убран `pb-[48px]` у правой панели).
- Убраны скругления снизу у бара (desktop и mobile): `rounded-t-[20px]`.
- Десктоп: верхний паддинг бара 24px (`pt-6`), нижний 32px + safe-area.

### Мобилка — паддинги раздела проектов
- Контейнер контента: `px-7 max-[740px]:px-5`, `pt-8` (как на главной).
